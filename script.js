...
function renderOrderScreen() {
    ...
    // Calcula e exibe o valor restante
    const remainingBalanceDisplay = document.getElementById('remainingBalanceDisplay');
    const paidTotal = calculatePaidTotal(); // CHAME A FUNÇÃO CORRETA AQUI
...
function updateChargeModalUI() {
    const subtotalDisplay = document.getElementById('finalSubtotalDisplay');
    ...
    if(subtotalDisplay) subtotalDisplay.textContent = `R$ ${finalCharge.subtotal.toFixed(2).replace('.', ',')}`;
    if(totalDisplay) totalDisplay.textContent = `R$ ${finalCharge.total.toFixed(2).replace('.', ',')}`;
    ...
    if (paymentSummaryList) {
        ...
        // Adiciona o 'Valor Restante' como o último item da lista.
        paymentsHtml += `
            <div class="flex justify-between items-center py-1 font-bold border-t border-gray-200 mt-2 pt-2">
                <span>VALOR RESTANTE:</span>
                <span id="remainingBalanceDisplay" class="text-red-600">R$ ${Math.max(0, remainingBalance).toFixed(2).replace('.', ',')}</span>
            </div>
        `;
...
function selectPaymentMethod(method) {
    ...
    // Ativa/desativa o botão de adicionar pagamento
    const addPaymentBtn = document.getElementById('addPaymentBtn');
    if (addPaymentBtn) {
        addPaymentBtn.disabled = false;
        addPaymentBtn.classList.remove('disabled:opacity-50');
    }
}
