// --- CONTROLLERS/CLIENTORDERCONTROLLER.JS (ATUALIZADO COM CORREÇÃO DE FILTROS) ---
...
// --- FUNÇÃO DE RENDERIZAR O MENU (CORRIGIDA) ---
function renderMenu(append = false) { 
    ...
    if (currentCategoryFilter !== 'all') {
        filteredProducts = products.filter(p => {
            // --- CORREÇÃO DE FILTRAGEM ---
            
            // 1. Verifica se a categoria principal (string) bate
            const hasCatString = p.category && p.category === currentCategoryFilter;

            // 2. Verifica se existe no array de categorias (se o WooCommerce enviou)
            const hasCatArray = p.categories && Array.isArray(p.categories) && p.categories.some(c => c.slug === currentCategoryFilter);
            
            // 3. Verifica se corresponde a um "Setor" (para botões que usam nome de setor)
            const isSector = p.sector && p.sector.toLowerCase() === currentCategoryFilter.toLowerCase();
            
            return hasCatArray || hasCatString || isSector;
            // -----------------------------
        });
    }
